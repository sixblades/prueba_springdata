...
function EvaluationController($rootScope, $sce, $state, Principal, $translate, QuestionsResource, LastEvaluationAssigned, GetLastAutoEvalByEmployeeId, LastEvaluationAssignedWithEvalType, StatesResource, AnswersSelect, AnswersEval, EmpleadosEvaluables, EvaluationEmployeeLeaderResource, AnswersResource, EmpleadosAE, EmployeeEvaluationResource, CompetencesResource, PillarsResource, isEvaluationDate, isFinalEvaluationDate, EmployeeIsLeaderResource, EmployeesRRHH, PrintService) {
					...
					GetLastAutoEvalByEmployeeId.query({ employeeId: vm.employeeId }, function (result) {
						if (result.length > 0) {
							...
							if (codigoResp != undefined) {
								...
								AnswersSelect.query({ id: codigoResp }, function (res) {
	...
	vm.loadAll = function () {
		if (Mentoring.isUndefinedOrNull(vm.evaluable.info)) {
			if (vm.isLeader === 0) {
				QuestionsResource.query({ leader: 0 }, function (result) {
		...
		// intenta autoevaluarse alguien no evaluable
			...
			// empleado se intenta autoevaluar cuando no es periodo de autoevaluacion
					...
					if (vm.validarNoSeObserva(datos)) {
						...
						AnswersEval.guardar2(evaluationToSave).then(function (obj) {
							var currentEvalId = obj.data.id;
							var respuestaJSON = AnswersEval.agrupar(datos, currentEvalId, vm.isFinalEvaluationDate);
							AnswersEval.guardar(respuestaJSON);

							var respuestaJSON3 = AnswersEval.agrupar3(currentEvalId, numDes, vm.destacados, vm.isFinalEvaluationDate);
							AnswersEval.guardar3(respuestaJSON3);

							var respuestaJSON4 = AnswersEval.agrupar3(currentEvalId, numMej, vm.mejorar, vm.isFinalEvaluationDate);
...
//# sourceMappingURL=evaluation.controller.refactor.js.map
